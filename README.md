...
4. **PySide6 Overlay** displays lyrics with a transparent, click-through, always-on-top window.
...
- Mouse clicks **pass through** the overlay — you can click buttons in your game normally.
- The overlay has no title bar, no taskbar icon, and doesn't steal focus.
...
| Font | Segoe UI, 16pt |
| Click behavior | Pass-through (WS_EX_TRANSPARENT) |
...
| `MAX_VISIBLE_LINES` | `overlay.py` | 8 | How many lyric lines to show |
| `FONT_SIZE` | `overlay.py` | 16 | Font size in points |
...
| Click-through not working | Ensure you're on Windows. Run the terminal as the same user as Firefox. |
...
- **pywin32** — Windows API (click-through, window enumeration)
